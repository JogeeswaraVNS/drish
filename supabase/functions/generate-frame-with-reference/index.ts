...
serve(async (req) => {
  ...
  try {
    ...
    // Use the product reference image
    if (gen.reference_image_url) {
      ...
      if (dlErr || !refData) {
        throw new Error("Could not download product reference image");
      }
      
      const buffer = await refData.arrayBuffer();
      const referenceImageBase64 = uint8ArrayToBase64(new Uint8Array(buffer));
      contentParts.push({
        inlineData: { mimeType: "image/png", data: referenceImageBase64 },
      });
    }

    console.log(`Generating frame ${frameNumber} with Gemini 3 Pro Image Preview (with product reference)...`);
